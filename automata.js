...
function validarString(maquina,string){ //demuestra que es AFD -- retorna 0 si es invalido el string , 1 en el caso contrario
  var aux=0; var aux2=1;
  for(var i=0; i<string.length;i++){ //recorre string
    for(var j=0;j<maquina.estados.length;j++){ //recorre cada estado
      for(var k=0;k<maquina.estados[j].opciones.length;k++){ //para recorrer las opciones de cada estado
        if(string[i]==maquina.estados[j].opciones[k] ){
            aux=1; //si alguna de las opciones de cada estado es igual retorna
        }
      }
      if(aux==0){ //al terminar de comparar una letra revisa si fue igual a alguna opcion
...
function incompatibles(maquina,nod1,nod2){ //retorna true si 2 estados son incompatibles --
   if(nod1.caracter==nod2.caracter){return true;} // true, estaria comparando un estado con si mismo por lo cual no se pueden trabajar como equivalentes
   for(var i=0; i<maquina.est_final.length;i++){ //recorre estados finales
      if(nod1.caracter==maquina.est_final[i]){return true;} //si nod1 es estado final, es incompatible con todos
      if(nod2.caracter==maquina.est_final[i]){return true;} //si nod2 es estado final, es incompatible con todos
   }
   for(var i=0; i<nod1.nomb_estados.length;i++){ //compara opciones
     for(var j=0; j<nod1.nomb_estados.length;j++){
         if(nod1.opciones[i]==nod2.opciones[j]){
             if(nod1.nomb_estados[i]!=nod2.nomb_estados[j]){ //si son iguales, no hay info
                  for(var k=0; k<maquina.est_final.length;k++){
                      if(nod1.nomb_estados[i]==maquina.est_final[k] || nod2.nomb_estados[j]==maquina.est_final[k]){return true;}
